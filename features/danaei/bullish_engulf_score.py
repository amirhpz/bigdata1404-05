...
def bullish_engulf_score_raw(df: pd.DataFrame, atr_period: int = 14) -> pd.Series:
    """
    Bullish engulf intensity: I[bull_engulf] * abs(close - open) / (ATR_14 + eps).
    """
    ...
    # current opens at or below prev close
    open_engulfs = df["open"] <= prev_close
    # current closes at or above prev open
    close_engulfs = df["close"] >= prev_open

    is_engulf = prev_bearish & curr_bullish & open_engulfs & close_engulfs
...
def build_feature_table(
    ...
    Output column:
    - bullish_engulf_score_scaled_pos_0_1
    """
    ...
    for _, g in grouped:
        ...
        raw = bullish_engulf_score_raw(g, atr_period=atr_period)
        final = class_a_pos_0_1(raw, norm_window=norm_window)

        g_out = g.copy()
        g_out["bullish_engulf_score_scaled_pos_0_1"] = final
        parts.append(g_out)
