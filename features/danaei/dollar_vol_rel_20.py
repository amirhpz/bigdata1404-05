...
def dollar_vol_rel_20_raw(df: pd.DataFrame, span: int = 20) -> pd.Series:
    """
    Dollar-volume relative to causal EMA of dollar-volume.

    dollar_vol_rel_20_t = (volume_t * close_t) / (EMA(volume * close, 20)_t + eps)
    """
    dv = df["volume"] * df["close"]
    ema_dv = ema(dv, span=span)
...
def build_feature_table(df: pd.DataFrame, span: int = 20, norm_window: int = 100) -> pd.DataFrame:
    ...
    Output column:
    - dollar_vol_rel_20_scaled_pos_0_1
    """
    ...
    for _, g in grouped:
        ...
        raw = dollar_vol_rel_20_raw(g, span=span)
        final = class_a_pos_0_1(raw, norm_window=norm_window)

        g_out = g.copy()
        g_out["dollar_vol_rel_20_scaled_pos_0_1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--span", type=int, default=20,
                   help="EMA span for dollar-volume baseline (default: 20).")
