...
FEATURE_COL = "atr_ratio_14_63_scaled_pos_0_1"


def compute_feature(g: pd.DataFrame, norm_window: int) -> pd.Series:
    raw = atr(g, period=14) / (atr(g, period=63) + EPS)
    return class_a_positive(raw, norm_window)


def main() -> None:
    p = argparse.ArgumentParser(description="Compute atr_ratio_14_63 (Class A positive).")
