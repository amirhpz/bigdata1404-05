...
FEATURE_COL = "inside_bar_compression_scaled_pos_0_1"
...
def compute_feature(g: pd.DataFrame, atr_period: int, norm_window: int) -> pd.Series:
    ...
    inside = ((g["high"] <= prev_high) & (g["low"] >= prev_low)).astype(float)
    raw = inside * (g["high"] - g["low"]) / (atr(g, period=atr_period) + EPS)

    return class_a_positive(raw, norm_window)


def main() -> None:
    p = argparse.ArgumentParser(description="Compute inside_bar_compression (Class A positive).")
