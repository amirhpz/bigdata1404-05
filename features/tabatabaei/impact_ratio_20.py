"""
Feature script: impact_ratio_20
...
Purpose:
- Return per unit *relative* volume:
    impact_ratio_20_raw = ret1 / ( V / (EMA(V,20) + eps) + eps )
- Interpretable as "price impact" / "efficiency":
    - Large |ret| on low relative volume -> high impact (inefficient / sensitive move)
    - Small |ret| on high relative volume -> low impact (efficient / absorbed move)

Raw formula details (causal):
- ret1_t = (close_t / close_{t-1}) - 1
- rel_vol_t = volume_t / (EMA(volume, 20)_t + eps)
- impact_ratio_raw_t = ret1_t / (rel_vol_t + eps)
...
Notes:
- All computation is per-symbol and symbol-local (no cross-symbol mixing).
- Early rows will be NaN due to EMA warm-up and normalization window warm-up.
"""
...
def impact_ratio_20(df: pd.DataFrame, vol_ema_span: int = 20) -> pd.Series:
    """
    Relative signed feature:
        impact_ratio = ret1 / ( volume / (EMA(volume, span) + eps) + eps )

    This is unitless and scale-invariant (volume normalized by its EMA).
    """
    r1 = ret_1(df)
    vol_ema = ema(df["volume"], span=vol_ema_span)

    eps = 1e-12
    rel_vol = df["volume"] / (vol_ema + eps)
    return r1 / (rel_vol + eps)
...
def build_feature_table(
    ...
    Output column:
    - impact_ratio_20_scaled_signed_m1_p1
    """
    ...
    for _, g in grouped:
        ...
        raw = impact_ratio_20(g, vol_ema_span=vol_ema_span)
        ...
        g_out = g.copy()
        g_out["impact_ratio_20_scaled_signed_m1_p1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute impact_ratio_20 (Class A) and map it to signed [-1, +1] with robust-z + Gaussian clip."
    )
...
def main() -> None:
    ...
    print("Columns added:")
    print(" - impact_ratio_20_scaled_signed_m1_p1")
