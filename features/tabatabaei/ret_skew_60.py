...
Purpose:
- Measure asymmetry (skewness) of 1-bar returns over the past 60 observations:
      skew(ret1, 60)
- Interpretable as downside-tail bias (negative skew) vs upside-tail bias (positive skew),
  then made fuzzy-ready via robust-z + Gaussian central clipping.

Raw feature (causal):
1) ret1_t = (close_t / close_{t-1}) - 1
2) ret_skew_60_raw_t = skew(ret1_{t-60}..ret1_{t-1})  (PAST-ONLY)

Important anti-leakage rule:
- Rolling skew is computed from past values only by applying shift(1) before rolling.
...
Notes:
- Early rows will be NaN due to return warm-up, skew lookback warm-up, and normalization warm-up.
...
def rolling_skew_past_only(series: pd.Series, window: int = 60) -> pd.Series:
    """
    Leakage-safe rolling skewness over a past-only window.

    Anti-leakage:
    - Uses series.shift(1) so current value is NOT included in the rolling window.
    """
    past = series.shift(1)
    # Pandas rolling skew uses an unbiased estimator; warm-up stays NaN with min_periods=window.
    return past.rolling(window=window, min_periods=window).skew()


def ret_skew_60(df: pd.DataFrame, lookback: int = 60) -> pd.Series:
    """
    Raw skewness feature:
        skew(ret1, lookback) computed past-only.
    """
    r1 = ret_1(df)
    return rolling_skew_past_only(r1, window=lookback)
...
def build_feature_table(
    ...
    Output column:
    - ret_skew_60_scaled_signed_m1_p1
    """
    ...
    for _, g in grouped:
        ...
        raw = ret_skew_60(g, lookback=lookback)
        ...
        g_out = g.copy()
        g_out["ret_skew_60_scaled_signed_m1_p1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute ret_skew_60 (Class A) and map it to signed [-1, +1] with robust-z + Gaussian clip."
    )
    parser.add_argument("--input", type=Path, required=True, help="Input CSV with OHLCV columns.")
    parser.add_argument("--output", type=Path, required=True, help="Output CSV path.")
    parser.add_argument("--lookback", type=int, default=60, help="Rolling skew lookback (default: 60).")
    parser.add_argument(
        "--norm-window",
        type=int,
        default=100,
...
def main() -> None:
    ...
    print("Columns added:")
    print(" - ret_skew_60_scaled_signed_m1_p1")
