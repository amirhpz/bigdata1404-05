"""
Feature script: trend_alignment_20_50_200
...
Purpose:
- Multi-horizon trend alignment via ATR-normalized cross-horizon product:
      ((EMA20 - EMA50) / (ATR14 + eps)) * ((EMA50 - EMA200) / (ATR14 + eps))
- Interpretable as:
    - positive: aligned bullish (EMA20>EMA50 and EMA50>EMA200) OR aligned bearish
               (EMA20<EMA50 and EMA50<EMA200) -> both gaps same sign => product > 0
    - negative: mixed / misaligned horizons (signs differ) => product < 0
  then made fuzzy-ready via robust-z + Gaussian central clipping.

Raw feature (causal):
1) EMA20_t, EMA50_t, EMA200_t computed from close (EMA, causal)
2) ATR14_t via EMA smoothing of True Range (causal)
3) gap_short_t = (EMA20_t - EMA50_t) / (ATR14_t + eps)
4) gap_long_t  = (EMA50_t - EMA200_t) / (ATR14_t + eps)
5) raw_t = gap_short_t * gap_long_t
...
def trend_alignment_20_50_200(df: pd.DataFrame, ema20: int = 20, ema50: int = 50, ema200: int = 200, atr_period: int = 14) -> pd.Series:
    """
    Relative signed feature:
        ((EMA20 - EMA50) / ATR14) * ((EMA50 - EMA200) / ATR14)
    """
    e20 = ema_close(df, span=ema20)
    e50 = ema_close(df, span=ema50)
    e200 = ema_close(df, span=ema200)
    atr_series = atr(df, period=atr_period)

    eps = 1e-12
    gap_short = (e20 - e50) / (atr_series + eps)
    gap_long = (e50 - e200) / (atr_series + eps)
...
def build_feature_table(
    ...
    Output column:
    - trend_alignment_20_50_200_scaled_signed_m1_p1
    """
    ...
    for _, g in grouped:
        ...
        g_out = g.copy()
        g_out["trend_alignment_20_50_200_scaled_signed_m1_p1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute trend_alignment_20_50_200 (Class A) and map it to signed [-1, +1] with robust-z + Gaussian clip."
    )
    parser.add_argument("--input", type=Path, required=True, help="Input CSV with OHLCV columns.")
    parser.add_argument("--output", type=Path, required=True, help="Output CSV path.")
    parser.add_argument("--ema20", type=int, default=20, help="EMA20 span (default: 20).")
    parser.add_argument("--ema50", type=int, default=50, help="EMA50 span (default: 50).")
    parser.add_argument("--ema200", type=int, default=200, help="EMA200 span (default: 200).")
...
def main() -> None:
    ...
    print("Columns added:")
    print(" - trend_alignment_20_50_200_scaled_signed_m1_p1")
