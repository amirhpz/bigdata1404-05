"""
Feature: ret_excess_kurt_60
Class: A (open-domain, signed)

Raw definition (past-only excess kurtosis):
    excess_kurtosis(ret1, 60)

Where:
    ret1_t = close_t / close_{t-1} - 1
    excess_kurtosis_t = kurtosis(ret1_{t-60}..ret1_{t-1})  (Fisher/excess, 0 for normal)

Implementation note:
- pandas rolling.kurt() returns Fisher's definition by default (excess kurtosis).
...
FEATURE_COL = "ret_excess_kurt_60_scaled_signed_m1_p1"
...
def compute_feature(g: pd.DataFrame, lookback: int, norm_window: int) -> pd.Series:
    ...
    # Past-only rolling excess kurtosis (Fisher) to avoid leakage.
    raw = ret1.shift(1).rolling(window=lookback, min_periods=lookback).kurt()
    return class_a_signed(raw, norm_window)


def main() -> None:
    p = argparse.ArgumentParser(description="Feature: ret_excess_kurt_60 (Class A signed)")
