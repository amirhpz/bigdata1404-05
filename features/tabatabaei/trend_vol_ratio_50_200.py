"""
Feature: trend_vol_ratio_50_200
...
FEATURE_COL = "trend_vol_ratio_50_200_scaled_signed_m1_p1"


def compute_feature(g: pd.DataFrame, fast: int, slow: int, atr_period: int, norm_window: int) -> pd.Series:
    """Compute feature values for one symbol slice and return final scaled column."""
    ema_fast = g["close"].ewm(span=fast, adjust=False, min_periods=fast).mean()
    ema_slow = g["close"].ewm(span=slow, adjust=False, min_periods=slow).mean()

    atr_n = atr(g, atr_period)
    raw = (ema_fast - ema_slow) / (atr_n + EPS)
    return class_a_signed(raw, norm_window)


def main() -> None:
    p = argparse.ArgumentParser(description="Feature: trend_vol_ratio_50_200 (Class A signed)")
    ...
    out = add_feature_per_symbol(
        df,
        FEATURE_COL,
        lambda g: compute_feature(g, args.fast, args.slow, args.atr_period, args.norm_window),
    )
