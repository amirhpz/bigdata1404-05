...
Where (vol is past-only):
    ...
    std(ret1,20)_t = std(ret1_{t-20}..ret1_{t-1})
...
FEATURE_COL = "mom_vol_norm_5_20_scaled_signed_m1_p1"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Feature: mom_vol_norm_5_20 (Class A signed)")
    p.add_argument("--input", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--mom-lookback", type=int, default=5)
