"""
Feature script: mom_vol_norm_20_60
...
Purpose:
- Medium momentum per long risk:
      (close / close[-20] - 1) / (std(ret1, 60) + eps)
- Interpretable as weak/normal/strong medium trend relative to longer-horizon return volatility,
  then made fuzzy-ready via robust-z + Gaussian central clipping.

Raw feature (causal):
1) mom_20_t = close_t / close_{t-20} - 1     (uses only past close)
2) ret1_t   = close_t / close_{t-1} - 1
3) vol_60_t = std(ret1_{t-60}..ret1_{t-1})  (PAST-ONLY)
4) raw_t = mom_20_t / (vol_60_t + eps)
...
def momentum_n(df: pd.DataFrame, n: int = 20) -> pd.Series:
    """
...
def rolling_std_past_only(series: pd.Series, window: int = 60) -> pd.Series:
    """
...
def mom_vol_norm_20_60(df: pd.DataFrame, mom_lookback: int = 20, vol_lookback: int = 60) -> pd.Series:
    """
...
def build_feature_table(
    ...
    Output column:
    - mom_vol_norm_20_60_scaled_signed_m1_p1
    """
    ...
    for _, g in grouped:
        ...
        raw = mom_vol_norm_20_60(g, mom_lookback=mom_lookback, vol_lookback=vol_lookback)
        ...
        g_out = g.copy()
        g_out["mom_vol_norm_20_60_scaled_signed_m1_p1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute mom_vol_norm_20_60 (Class A) and map it to signed [-1, +1] with robust-z + Gaussian clip."
    )
    parser.add_argument("--input", type=Path, required=True, help="Input CSV with OHLCV columns.")
    parser.add_argument("--output", type=Path, required=True, help="Output CSV path.")
    parser.add_argument("--mom-lookback", type=int, default=20, help="Momentum lookback (default: 20).")
    parser.add_argument("--vol-lookback", type=int, default=60, help="Volatility std lookback (default: 60).")
    parser.add_argument(
        "--norm-window",
        type=int,
        default=100,
...
def main() -> None:
    ...
    print("Columns added:")
    print(" - mom_vol_norm_20_60_scaled_signed_m1_p1")
