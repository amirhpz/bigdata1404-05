"""
Feature script: breakout_strength_20_14

Class: A (positive-domain)
Raw:
    prev20_high_t = rolling_max(high_{t-1}, 20)
    x_t = max(0, close_t - prev20_high_t) / ATR14_t

ATR14 computed past-only (EMA on TR_{t-1})
...
def rolling_max_past_only(series: pd.Series, window: int) -> pd.Series:
    return series.shift(1).rolling(window=window, min_periods=window).max()


def raw_breakout_strength_20_14(df: pd.DataFrame) -> pd.Series:
    prev20_high = rolling_max_past_only(df["high"], window=20)
    atr14 = atr_ema_past_only(df, period=14)
    return np.maximum(0.0, df["close"] - prev20_high) / (atr14 + EPS)
...
def build_feature_table(df: pd.DataFrame, norm_window: int = 100) -> pd.DataFrame:
    ...
    for _, g in grouped:
        ...
        raw = raw_breakout_strength_20_14(g)
        final = class_a_pos_0_1(raw, norm_window=norm_window)

        g_out = g.copy()
        g_out["breakout_strength_20_14_scaled_pos_0_1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute breakout_strength_20_14 feature (Class A) scaled to [0,1].")
