"""
Feature script: tr_to_atr_14

Class: A (positive-domain)
Raw:
    TR_t / ATR14_t
ATR14 computed past-only as EMA(TR_{t-1},14)

Output column:
- tr_to_atr_14_scaled_pos_0_1
"""
...
def raw_tr_to_atr_14(df: pd.DataFrame) -> pd.Series:
    tr = true_range(df)
...
def build_feature_table(df: pd.DataFrame, norm_window: int = 100) -> pd.DataFrame:
    ...
    for _, g in grouped:
        ...
        raw = raw_tr_to_atr_14(g)
        final = class_a_pos_0_1(raw, norm_window=norm_window)

        g_out = g.copy()
        g_out["tr_to_atr_14_scaled_pos_0_1"] = final
        parts.append(g_out)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute tr_to_atr_14 feature (Class A) scaled to [0,1].")
