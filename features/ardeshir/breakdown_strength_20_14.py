"""
Feature: breakdown_strength_20_14
Class: A (positive-domain)

Raw:
    prev20_low = rolling_min(low.shift(1), 20)   # past support
    x = max(0, prev20_low - close) / ATR(14)
...
FEATURE_COL = "breakdown_strength_20_14_scaled_pos_0_1"


def compute_feature(df: pd.DataFrame, norm_window: int = 100) -> pd.Series:
    prev20_low = df["low"].shift(1).rolling(window=20, min_periods=20).min()
    atr14 = atr(df, period=14)
    raw = np.maximum(0.0, prev20_low - df["close"]) / (atr14 + EPS)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute breakdown_strength_20_14 (Class A) -> [0,1].")
