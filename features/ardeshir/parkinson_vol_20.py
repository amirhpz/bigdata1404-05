...
def compute_feature(df: pd.DataFrame, norm_window: int = 100) -> pd.Series:
    ...
    r2_mean = (r * r).rolling(window=20, min_periods=20).mean()
    raw = np.sqrt(r2_mean / (4.0 * np.log(2.0) + EPS))
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute parkinson_vol_20 (Class A) -> [0,1].")
