...
from features.feature_utils import true_range, atr, class_a_positive, add_feature_per_symbol, EPS
...
def compute_feature(df: pd.DataFrame, norm_window: int = 100) -> pd.Series:
    ...
    y = tr / (atr14 + EPS)
    std20 = y.rolling(window=20, min_periods=20).std()
    ema20 = y.ewm(span=20, adjust=False, min_periods=20).mean()
    raw = std20 / (ema20 + EPS)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute vol_of_vol_ratio_14_20 (Class A) -> [0,1].")
